...
func TestValidateInstallSecrets(t *testing.T) {
	...
	t.Run("missing secret", func(t *testing.T) {
		t.Parallel()
		err := secrets.ValidateInstallSecrets(pkg, "admin", nil, stubVerifier{})
		require.ErrorIs(t, err, secrets.ErrInstallSecretMissing)
	})

	t.Run("invalid password", func(t *testing.T) {
		t.Parallel()
		err := secrets.ValidateInstallSecrets(pkg, "admin", map[string]string{
...
func TestRequiredKeys(t *testing.T) {
	...
	specs := secrets.RequiredKeys([]model.Package{
...
func TestValidateInstallSecrets_NoSecretsConfigured(t *testing.T) {
	...
	err := secrets.ValidateInstallSecrets(model.Package{}, "admin", nil, stubVerifier{err: errors.New("unused")})
	require.NoError(t, err)
}
