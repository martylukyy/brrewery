...
func waitForRestart(t *testing.T, f *fixture) {
	t.Helper()
	require.Eventually(t, func() bool { return f.rec.has(restartCmd) }, 5*time.Second, 10*time.Millisecond,
		"update never issued the restart command")
}
...
func TestUpdateHappyPath(t *testing.T) {
	...
	assert.Equal(t, []string{"nginx -t", "systemctl reload nginx", "systemctl daemon-reload", restartCmd}, calls)

	// The job stays running until the restarted process reconciles it.
	current, ok := f.store.Get(job.ID)
	require.True(t, ok)
	assert.Equal(t, model.JobStatusRunning, current.Status)
